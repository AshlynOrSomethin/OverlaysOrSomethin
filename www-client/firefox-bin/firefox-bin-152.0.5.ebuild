...
DESCRIPTION="Mozilla Firefox (AudioStreamPatcher binary .deb)"
HOMEPAGE="https://github.com/AshlynOrSomethin/AudioStreamPatcher"
SRC_URI="
  amd64? ( https://github.com/AshlynOrSomethin/AudioStreamPatcher/releases/download/browsers-latest/firefox-patched-${PV}-amd64.deb -> ${P}_amd64.deb )
"
...
src_install() {
  ...
  if [[ -f ${D}/usr/share/applications/firefox.desktop ]]; then
    sed -i \
      -e 's|^Exec=.*|Exec=/usr/bin/firefox %u|' \
      -e 's|^Icon=.*|Icon=firefox|' \
      "${D}/usr/share/applications/firefox.desktop" || die
  fi

  if [[ ! -x ${D}/usr/bin/firefox ]] && [[ -x ${D}/opt/firefox/firefox ]]; then
    dosym /opt/firefox/firefox /usr/bin/firefox
  fi

  if [[ -x ${D}/usr/bin/firefox ]]; then
    dosym /usr/bin/firefox /usr/bin/firefox-bin
  fi
}
