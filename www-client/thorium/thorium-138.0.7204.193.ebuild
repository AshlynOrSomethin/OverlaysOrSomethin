...
PATCH_V="${PV%%\.*}-1"
THORIUM_COMMIT_ID="14e595802e7c70f837b88a135efb2ed18af0dabd"
THORIUM_WD="${WORKDIR}/thorium-${THORIUM_COMMIT_ID}"
UGC_PVR="138.0.7204.183-1"
...
SRC_URI="https://github.com/AshlynOrSomethin/gentoo-chromium-source-tarball/releases/download/${PV}/chromium-${PV}.tar.xz
	test? (
		https://github.com/AshlynOrSomethin/gentoo-chromium-source-tarball/releases/download/${PV}/chromium-${PV}-testdata.tar.xz -> chromium-${PV}-testdata-gentoo.tar.xz
...
src_prepare() {
	...
	if use custom-cflags; then
		# Use the full, unmodified patch
		PATCHES+=( "${FILESDIR}/chromium-138-compiler.patch" )
	else
		# Use a trimmed copy to avoid default_stack_frames section
		cp "${FILESDIR}/chromium-138-compiler.patch" \
			"${T}/chromium-138-compiler.patch" || die
		sed -i '/default_stack_frames/Q' \
			"${T}/chromium-138-compiler.patch" || die
		PATCHES+=( "${T}/chromium-138-compiler.patch" )
	fi
